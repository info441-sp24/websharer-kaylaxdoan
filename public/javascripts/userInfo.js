...
async function saveUserInfo(){
    ...
    try {
        ...
        let pronouns = document.getElementById("pronouns-input").value;

        let responseJson = await fetchJSON(`api/${apiVersion}/userInfos`, {
            method: "POST",
            body:{pronouns: pronouns, bio: bio}
        })
...
async function loadUserInfo(){
    ...
    try {
        const response = await fetch(`/api/${apiVersion}/userInfos?username=${encodeURIComponent(username)}`);
        const userInfo = await response.json();
        document.getElementById("pronouns-display").innerText = userInfo.pronouns;
        document.getElementById("bio-display").innerText = userInfo.bio;
